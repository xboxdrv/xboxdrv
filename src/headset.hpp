...
private:
  ...
  /** Non-blocking fd for Pulse/PipeWire pipe-sink PCM (S16LE @ 8 kHz mono). */
  int m_pulse_play_fd;
...
public:
  ...
  /** Create FIFOs and load PulseAudio/PipeWire pipe-source (16 kHz mic) and
      pipe-sink (8 kHz speaker). Requires `pactl` on PATH. */
