...
CommandLineParser::init_ini(Options* opts)
{
  ...
  m_ini.section("xboxdrv")
    ...
    ("rumble", &opts->rumble)
    ("led", std::bind(&Options::set_led, opts, std::placeholders::_1))
    ...
    ("config", std::bind(&CommandLineParser::read_config_file, this, std::placeholders::_1))
    ("alt-config", std::bind(&CommandLineParser::read_alt_config_file, this, std::placeholders::_1))
    ("timeout", &opts->timeout)
    ("priority", std::bind(&Options::set_priority, opts, std::placeholders::_1))
    ...
    ("toggle", std::bind(&Options::set_toggle_button, opts, std::placeholders::_1))
    ("ff-device", std::bind(&Options::set_ff_device, opts, std::placeholders::_1))

    ("deadzone", std::bind(&CommandLineParser::set_deadzone, this, std::placeholders::_1))
    ("deadzone-trigger", std::bind(&CommandLineParser::set_deadzone_trigger, this, std::placeholders::_1))
    ...
    ("dpad-rotation", std::bind(&CommandLineParser::set_dpad_rotation, this, std::placeholders::_1))

    // uinput stuff
    ("device-name",       std::bind(&Options::set_device_name, opts, std::placeholders::_1))
    ("device-usbid",      std::bind(&Options::set_device_usbid, opts, std::placeholders::_1))
    ...
    ("force-feedback",    std::bind(&Options::set_force_feedback, opts, std::placeholders::_1))
  ...
  m_ini.section("xboxdrv-daemon")
    ...
    ("dbus", std::bind(&Options::set_dbus_mode, opts, std::placeholders::_1))
    ("pid-file",      &opts->pid_file)
    ("on-connect",    &opts->on_connect)
    ("on-disconnect", &opts->on_disconnect)
    ;

  m_ini.section("modifier",     std::bind(&CommandLineParser::set_modifier,     this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("ui-buttonmap", std::bind(static_cast<void (CommandLineParser::*)(const std::string& name, const std::string& value)>
                                          (&CommandLineParser::set_ui_buttonmap), this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("ui-axismap",   std::bind(static_cast<void (CommandLineParser::*)(const std::string& name, const std::string& value)>
                                          (&CommandLineParser::set_ui_axismap), this, std::placeholders::_1, std::placeholders::_2));

  m_ini.section("buttonmap", std::bind(&CommandLineParser::set_buttonmap, this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("axismap",   std::bind(&CommandLineParser::set_axismap,   this, std::placeholders::_1, std::placeholders::_2));

  m_ini.section("autofire",   std::bind(&CommandLineParser::set_autofire, this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("relative-axis",   std::bind(&CommandLineParser::set_relative_axis, this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("calibration",   std::bind(&CommandLineParser::set_calibration, this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("axis-sensitivity",   std::bind(&CommandLineParser::set_axis_sensitivity, this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("device-name", std::bind(&CommandLineParser::set_device_name, this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("device-usbid", std::bind(&CommandLineParser::set_device_usbid, this, std::placeholders::_1, std::placeholders::_2));
  ...
  m_ini.section("evdev-absmap", std::bind(&CommandLineParser::set_evdev_absmap, this, std::placeholders::_1, std::placeholders::_2));
  m_ini.section("evdev-keymap", std::bind(&CommandLineParser::set_evdev_keymap, this, std::placeholders::_1, std::placeholders::_2));
}

void
...
CommandLineParser::apply_opt(ArgParser::ParsedOption const& opt, Options& opts)
{
  switch (opt.key)
  {
    ...
    case OPTION_MODIFIER:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_modifier, this, std::placeholders::_1, std::placeholders::_2));
      break;

    case OPTION_BUTTONMAP:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_buttonmap, this, std::placeholders::_1, std::placeholders::_2));
      break;

    case OPTION_AXISMAP:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_axismap, this, std::placeholders::_1, std::placeholders::_2));
      break;
    ...
    case OPTION_DEVICE_USBIDS:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_device_usbid, this, std::placeholders::_1, std::placeholders::_2));
      break;
    ...
    case OPTION_DEVICE_NAMES:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_device_name, this, std::placeholders::_1, std::placeholders::_2));
      break;
    ...
    case OPTION_UI_AXISMAP:
      process_name_value_string(opt.argument, std::bind(static_cast<void (CommandLineParser::*)(const std::string& name, const std::string& value)>
                                                        (&CommandLineParser::set_ui_axismap), this, std::placeholders::_1, std::placeholders::_2));
      break;
    ...
    case OPTION_EVDEV_ABSMAP:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_evdev_absmap, this, std::placeholders::_1, std::placeholders::_2));
      break;

    case OPTION_EVDEV_KEYMAP:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_evdev_keymap, this, std::placeholders::_1, std::placeholders::_2));
      break;
    ...
    case OPTION_AUTOFIRE:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_autofire, this, std::placeholders::_1, std::placeholders::_2));
      break;

    case OPTION_CALIBRARIOTION:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_calibration, this, std::placeholders::_1, std::placeholders::_2));
      break;

    case OPTION_RELATIVE_AXIS:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_relative_axis, this, std::placeholders::_1, std::placeholders::_2));
      break;

    case OPTION_AXIS_SENSITIVITY:
      process_name_value_string(opt.argument, std::bind(&CommandLineParser::set_axis_sensitivity, this, std::placeholders::_1, std::placeholders::_2));
      break;
