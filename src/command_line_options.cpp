...
enum {
  ...
  OPTION_NO_AUTO_POWEROFF,
  OPTION_NO_GUIDE_POWEROFF,
  OPTION_GUIDE_POWEROFF_TIMEOUT,
  OPTION_TIMEOUT,
  OPTION_HEADSET,
  OPTION_HEADSET_DUMP,
  OPTION_HEADSET_PLAY,
...
CommandLineParser::init_argp(int argc, char** argv)
{
  ...
  m_argp.add_group("Headset Options (experimental, Xbox360 USB only): ")
    ...
    .add_option(OPTION_HEADSET_PLAY_WAV, NUL, "headset-play-wav", "FILE",  "Play PCM WAV FILE on headset (resampled to 16 kHz mono, G.726-32 encoded)");
  ...
  switch (opt.key)
    {
      ...
      case OPTION_HEADSET_WAV:
        opts.headset = true;
        opts.headset_wav = opt.argument;
        break;
...
{
  std::cout
    << "xboxdrv " << xboxdrv_version() << " - https://github.com/xboxdrv/xboxdrv\n"
